...
const toUnderscored = obj => {
  _lodash.default.forEach(obj, (k, v) => {
...
const calcDelta = (current, next, exclude, strict) => {
  ...
  const di = (0, _deepDiff.diff)(current, next);
  if (DEBUG) {
    console.log('di', di);
    let diffs2 = [];
    if (di) {
      diffs2 = di.map(i => JSON.parse(JSON.stringify(i).replace('"__data",', '')));
    }
  ...
  let diffs = [];
  if (di) {
    diffs = di.map(i => JSON.parse(JSON.stringify(i).replace('"__data",', ''))).filter(i => {
