...
exports.init = (sequelize, optionsArg) => {
  ...
  const options = _lodash.default.defaults(optsArg, defaultOptions);
  ...
  function createAfterHook(operation) {
    const afterHook = function afterHook(instance, opt) {
      ...
      if (instance.context && (instance.context.delta && instance.context.delta.length > 0 || destroyOperation)) {
        ...
        query[options.defaultAttributes.userId] = ns && ns.get(options.continuationKey) || opt.userId;
        query[options.defaultAttributes.documentId] = instance[instance._modelOptions.paperTrailDocumentKey || 'id'];
        ...
        return revision.save({
          ...
          if (options.enableRevisionChangeModel) {
            _lodash.default.forEach(delta, difference => {
              ...
              const n = _helpers.default.diffToString(difference.item ? difference.item.rhs : difference.rhs);
  ...
  _lodash.default.assignIn(_sequelize.default.Model, {
    hasPaperTrail: function hasPaperTrail(args = {}) {
      if (options.debug) {
      ...
      //overwritting the document_id primary key mapping column
      this.options.paperTrailDocumentKey = args.paperTrailDocumentKey || 'id';
  ...
  return {
    ...
    defineModels: function defineModels(db) {
      ...
      attributes[options.defaultAttributes.userId] = {
